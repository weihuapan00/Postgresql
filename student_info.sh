...
echo "$($PSQL "SELECT major from students full join majors on students.major_id = majors.major_id where major is not null and (first_name ilike '%ma%' or student_id is null) order by major;")"
